...
from .db import connect_db, resolve_draft_id, upsert_fts
...
UNCERTAINTY_MARKERS = ["кажется", "думаю", "возможно", "пока", "не уверен", "current", "guess"]
REASONING_MARKERS = ["потому что", "по итогу", "вывод", "в чем суть", "because", "broke"]
...
def _score_auto_example(text: str, reactions: int) -> int:
    ...
    score += 1 if any(marker in lowered for marker in ["я ", "у меня", "сейчас", "i "]) else 0
