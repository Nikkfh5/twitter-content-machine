...
- адаптивный формат: коротко только если мысль маленькая, длиннее если есть что раскрыть;
...
`--short` писать нужно только если ты явно хочешь короткий пост.
Без флага работает adaptive-режим: он может сделать нормальный single post на
несколько коротких абзацев, если в мысли есть мясо.
